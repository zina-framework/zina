...
abstract Color(Vector<Float>) from Vector<Float> to Vector<Float> {
    ...
    public inline function copyFrom(color:Color):Color {
        r = color.r;
        g = color.g;
        b = color.b;
        a = color.a;
	...
	@:from
    public static inline function fromString(color:String):Color {
        color = color.trim();

        if(!_COLOR_REGEX.match(color))
            return BLACK;

        var hexColor:String = "0x" + _COLOR_REGEX.matched(2);
        var result:Color = Color.fromInt(Std.parseInt(hexColor));
    ...
    private static final _COLOR_REGEX:EReg = ~/^(0x|#)(([A-F0-9]{2}){3,4})$/i;
